...
{
    $pesanan = Pemesanan::with('pelanggan')
        ->where('status', 'menunggu_tagihan_awal')
        ->get();

    return view('payment.tagihan.awal', compact('pesanan'));
}

public function kirimTagihanAwal(Pemesanan $pemesanan)
{
    Tagihan::create([
    ...
    return back()->with('success', 'Tagihan awal berhasil dikirim');
}
